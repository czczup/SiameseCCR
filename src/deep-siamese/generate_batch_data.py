...
def get_negative_pair():  # 获取负样本对
    index1 = np.random.randint(0, 3755)  # 随机产生汉字的编号
    index2 = np.random.randint(0, 3755)  # 随机产生汉字的编号
    while index2 == index1:
        index2 = np.random.randint(0, 3755)  # 随机产生汉字的编号
    assert index1 != index2, "index1 can not equal to index2"
    image1_index = np.random.randint(0, 4)
    image2_index = np.random.randint(0, 4)
    path1 = config.train_image_path+"/"+characters[index1].filenames[image1_index]
    path2 = config.train_image_path+"/"+characters[index2].filenames[image2_index]
    image1 = cv2.imread(path1, cv2.IMREAD_GRAYSCALE)
    image2 = cv2.imread(path2, cv2.IMREAD_GRAYSCALE)
    stroke = str(characters[index1].stroke_number)+"_"+str(characters[index2].stroke_number)
    return image1, image2, stroke
