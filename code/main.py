...
def main():
    ...
    while True:
        print(train_time)
        sess, saver, siamese = init_model()
        if not os.path.exists("../result/log/train%d.log" % train_time):
            train(sess, saver, siamese, train_time, data_sum=sample_sum*2, epoch=1)
        if not os.path.exists("../result/result-train%d.csv" % train_time):
            test(siamese, sess, dataset="train", train_time=train_time)
        if not os.path.exists("../result/result-test-seen%d.csv" % train_time):
            test(siamese, sess, dataset="test-seen", train_time=train_time)
        if not os.path.exists("../result/result-test-unseen%d.csv" % train_time):
            test(siamese, sess, dataset="test-unseen", train_time=train_time)
        if not os.path.exists("../result/result-test%d.csv" % train_time):
